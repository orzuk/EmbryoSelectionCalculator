...
n  <- 5
Var1 <- (n-1) * integrate(gauss_poly, -8, 8, n=c(n-2,2,1))$value # var1 = (n-1) .* quad( @(t) pnorm(t).^(n-2) .* dnorm(t).^2.*t, -8, 8)
Var2 <- -1/n + integrate(gauss_poly, -8, 8, n=c(n-1,1,2))$value # var2 = -1/(n-0) + quad( @(t) pnorm(t).^(n-1) .* dnorm(t).*t.^2, -8, 8)
var_const <- integrate(gauss_poly, -5, 5, n=c(n-1,1,0))$value # var_const =  quad( @(t) pnorm(t).^(n-1) .* dnorm(t), -5, 5) % should be 1/(n-1) 
...
variance.decompose <- FALSE
if(variance.decompose)
{
  ...
  for (n in c(1:max_n))
  {
    G_exact = gain_moments(n, 1, 'exact', subtract_mean, sib.flag) # divide to mean and variance
    E_G_exact[n] <- G_exact$E.G
    Var_G_exact[n] <- G_exact$Var.G  
    G_approx = gain_moments(n, 1, 'approx', subtract_mean, sib.flag)
