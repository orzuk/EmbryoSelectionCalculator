...
library(ggplot2)
library(lqmm)
library(mvtnorm) # For the MVN integration approach
...
simulate_PGS_disease_risk <- function(Z, K, E, n_sims = 1000) {
  threshold <- qnorm(1-K)
  samples <- mvrnorm(n_sims, mu = Z, Sigma = E) # generate a ton of samples with mean Z
  ...
  R_list <- list(R_0, R) # concatenate the two lists and return
  return(R_list)
}
...
Z_frame <- expand.grid(z = seq(0, 3, 0.1), K = K, PGS = seq(0.05, 0.7, 0.05), R_0 = 0, R_1 = 0, R_2 = 0, R_3 = 0, R_4 = 0, R_5 = 0, MV_CDF_R0 = 0)
for(i in 1:nrow(Z_frame)) {
  ...
  Z_frame$R_4[i] <- R[4]
  Z_frame$R_5[i] <- R[5]
  Z_frame$MV_CDF_R0[i] = pmvnorm(lower = -Inf, upper = K, mean = Z, sigma = E)
}

# Integration using pmvnorm() doesn't seem to work too well.

ggplot(data = Z_frame, aes(x = z, y = R_2)) + geom_point(aes(color = PGS)) + facet_wrap(~K, nrow =3) +
...
ggplot(data = Z_frame, aes(x = z, y = R_0)) + geom_point(aes(color = PGS)) + facet_wrap(~K, nrow =3) + 
  ggtitle("P(Disease Free) as a Function of Z, K, and h^2_{pgs}") + xlab("Z-score of Embryo") + 
  ylab("P(Disease Free)") + scale_color_gradient(name = "h^2_{pgs}")

# Take a subset of the data:
subset_0.1 <- subset(Z_frame, K == 0.1)
